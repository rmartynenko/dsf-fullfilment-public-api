...
Функция ОбъектXdtoПоДаннымВходящегоЗапроса(ТелоЗапроса, Ошибки = Неопределено) Экспорт
	...
	// во входящем узле <root> необходимо принудительно удалить все атрибуты элемента
	// напр., <root xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance""> 
	// должно стать <root>

	СтрокаРегулярногоВыражения = "<root\s+([^>]*)"; // Поиск <root message="Hello">, вернёт "<root message="Hello"
	Результат = СтрНайтиПоРегулярномуВыражению(СтрокаXML, СтрокаРегулярногоВыражения);	
	СтрокаПоиска = Результат.Значение;

//	УдаляемыеПространстваИмен = "<root xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">";

	СтрокаXML = СтрЗаменить(СтрокаXML, СтрокаПоиска, "<root"); // 
	...
	Попытка
		НовыйОбъектXDTO = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ТипXDTO);
...
Функция ОписаниеЭлементаСпискаТоваровВРеестре(ОбъектXdto) Экспорт
	...
	Если ОбъектXdto["cargoTypes"] <> Неопределено Тогда
	...
	Если ОбъектXdto["barcodes"] <> Неопределено Тогда
