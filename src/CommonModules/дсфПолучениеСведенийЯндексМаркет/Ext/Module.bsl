...
Функция НовоеОписаниеОстатковТовара() Экспорт
	...
	ОстаткиТовара.Вставить("ИдентификаторПоставщика", 0);
...
//   - 
//
Функция СведенияОбОстаткахТоваров(КоличествоЗаписей = 200,
										Смещение = 0,
	...
	Если ТаблицаНоменклатураДляОтбора = Неопределено Или ТаблицаНоменклатураДляОтбора.Количество() = 0 Тогда
		ТекстЗапроса = "ВЫБРАТЬ
		               ...
		               |ПОМЕСТИТЬ ВТ_ТаблицаНоменклатурыПолная
		               |ИЗ
		               ...
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ ПЕРВЫЕ %ВеличинаСдвига%
		               |	ВТ_ТаблицаНоменклатурыПолная.НоменклатураСсылка КАК НоменклатураСсылка,
		               |	ВТ_ТаблицаНоменклатурыПолная.Артикул КАК Артикул
		               |ПОМЕСТИТЬ ВТ_ТаблицаНоменклатурыСдвиг
		               |ИЗ
		               |	ВТ_ТаблицаНоменклатурыПолная КАК ВТ_ТаблицаНоменклатурыПолная
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	НоменклатураСсылка
		               |
		               ...
		               |ИЗ
		               |	ВТ_ТаблицаНоменклатурыПолная КАК ВТ_ТаблицаНоменклатурыПолная
		               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаНоменклатурыСдвиг КАК ВТ_ТаблицаНоменклатурыСдвиг
		               |		ПО ВТ_ТаблицаНоменклатурыПолная.НоменклатураСсылка = ВТ_ТаблицаНоменклатурыСдвиг.НоменклатураСсылка
		               |ГДЕ
		               |	ВТ_ТаблицаНоменклатурыСдвиг.НоменклатураСсылка ЕСТЬ NULL
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ВТ_ТаблицаНоменклатурыПолная.НоменклатураСсылка
		               |
		               |ИНДЕКСИРОВАТЬ ПО
		               |	НоменклатураСсылка
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |УНИЧТОЖИТЬ ВТ_ТаблицаНоменклатурыПолная
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |УНИЧТОЖИТЬ ВТ_ТаблицаНоменклатурыСдвиг";
		
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ВеличинаСдвига%", Формат(Смещение, "ЧДЦ=0; ЧН=; ЧГ=0"));
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%КоличествоЗаписей%", Формат(КоличествоЗаписей, "ЧДЦ=0; ЧН=; ЧГ=0"));
	...
	|	ВТ_ТаблицаНоменклатуры.НоменклатураСсылка КАК НоменклатураСсылка,
	|	ВТ_ТаблицаНоменклатуры.Артикул КАК Артикул
	...
	|		ПО ОбъектыХранения.Номенклатура = ВТ_ТаблицаНоменклатуры.НоменклатураСсылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Остатки.ОбъектХранения КАК ОбъектХранения,
	|	ВТ_ОбъектыХранения.НоменклатураСсылка КАК НоменклатураСсылка,
	...
	|		ПО ВТ_ОбъектыХранения.ОбъектХранения = Остатки.ОбъектХранения
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОбъектХранения
	|ИТОГИ
	...
	РезультатПоОстаткам = Результат[1];
