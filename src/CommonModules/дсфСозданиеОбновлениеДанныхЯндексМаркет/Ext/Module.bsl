...
Функция НовоеОписаниеШтрихкода() Экспорт
	Возврат дсфНоменклатураАпи.НовоеОписаниеШтрихкода();
	
КонецФункции

// устарела. Использовать дсфНоменклатураАпи.НовоеОписаниеШтрихкода
Функция НовоеОписаниеВГХОбъекта() Экспорт
	Возврат дсфНоменклатураАпи.НовоеОписаниеВГХОбъекта();
...
Функция СтрокаПлановПоступленияЗначенияСправочников() Экспорт
...
Процедура СоздатьОбновитьЗаявкуНаПриемку(ДанныеЗаявки, РезультатОбработки) Экспорт
	...
	ЗаявкаНаПриемку = 
		РегистрыСведений.дсфИдентификаторыЯндексМаркет.СсылкаНаОбъектПоПубличномуИдентификатору(ИдентификаторВнешний);
	...
	Попытка
		...
		Иначе
			...
			РегистрыСведений.дсфСвязанныеОбъектыЗаявкаНаПриемкуЯндексМаркет.ДобавитьНовуюЗапись(СсылкаНаНовый);
...
Функция РезультатОбработкиРеестраПриемки() Экспорт
	ОписаниеРезультата = Новый Структура;
...
Процедура СоздатьОбновитьРеестрПриемки(ДанныеРеестра, РезультатОбработки) Экспорт
	...
	ПланПоступления = 
		РегистрыСведений.дсфСвязанныеОбъектыЗаявкаНаПриемкуЯндексМаркет.ПланПоступленияПоЗаявкеНаПриемку(ЗаявкаНаПриемку);
	...
	Попытка
		...
		Если Не ЗаписатьСтрокиПлановПоступленияГрузовыхМест(ПланПоступления, ДанныеРеестра["ГрузовыеМеста"]) Тогда
		...
		ЭлементБлокировки = БлокировкаДанныхРегистров.Добавить("РегистрСведений.дсфСвязанныеОбъектыЗаявкаНаПриемкуЯндексМаркет");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		...
		// dsf_СвязанныеОбъектыЗаявкаНаПриемку
		РегистрыСведений.дсфСвязанныеОбъектыЗаявкаНаПриемкуЯндексМаркет.СвязатьПланПоступленияСЗаявкойНаПриемку(ПланПоступления,
																									ЗаявкаНаПриемку);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(дсфВходящиеЗапросыЯндексМаркет.ИмяСобытияЖурналаРегистрации(),
								УровеньЖурналаРегистрации.Ошибка,
								Метаданные.HTTPСервисы.дсфYandexMarketFulfillment,,
								ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ЗаписатьСтрокиПлановПоступления(ПланПоступления, ТоварныйСостав, ПричинаКорректировки = Неопределено)
	...
	Для каждого СтрокаТоварногоСостава Из ТоварныйСостав Цикл
		...
		ЭлементыСправочников = СтрокаПлановПоступленияЗначенияСправочников();
		
		Если Не ОпределитьЭлементыСправочниковДляСтрокиПланаПоступления(СтрокаТоварногоСостава,
...
Процедура ОбновитьШтрихкодыНоменклатуры(Номенклатура, Штрихкоды, Упаковка = Неопределено) Экспорт
	дсфНоменклатураАпи.ОбновитьШтрихкодыНоменклатуры(Номенклатура, Штрихкоды, Упаковка);
...
//   - Булево
//
Функция ОпределитьЭлементыСправочниковДляСтрокиПланаПоступления(ОписаниеТовара, ЭлементыСправочников, Ошибки) Экспорт
	...
	// в дальнейщем переделать на хранение идентификаторов номенклатуры в регистре
	// если длина артикула будет превышать 25 символов
	Номенклатура = Справочники.Номенклатура.НайтиПоРеквизиту("Артикул", АртикулНоменклатуры);
