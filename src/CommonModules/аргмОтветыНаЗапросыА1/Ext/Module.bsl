...
Функция ОписаниеОтвета(РезультатОбработкиЗапроса) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОбработкиЗапроса.ДатаОбработки) Тогда
		ОписаниеОтвета.Заголовки.Вставить("X-1C-Timestamp", XMLСтрока(РезультатОбработкиЗапроса.ДатаОбработки));

	КонецЕсли;
	
	Если ЗначениеЗаполнено(РезультатОбработкиЗапроса.ДатаУстаревания) Тогда
		ОписаниеОтвета.Заголовки.Вставить("X-1C-Expired", XMLСтрока(РезультатОбработкиЗапроса.ДатаУстаревания));
...
Функция ОтветИсключениеВоВремяОбработкиЗапроса(Ошибки = Неопределено) Экспорт
	...
	ЗаголовкиHTTP.Вставить("X-1C-Error", "Exception during request processing");
