...
Функция СодержимоеУзлаResponse(СодержимоеОтвета, ТипЗапроса, Ошибки) Экспорт
	...
	ИначеЕсли ТипЗапроса = "getInboundStatus" Тогда
		ОтветXdto["inboundStatuses"] = СодержимоеОтвета;
	
	ИначеЕсли ТипЗапроса = "getInboundsStatus" Тогда
		ОтветXdto["inboundsStatus"] = СодержимоеОтвета;
		
	ИначеЕсли ТипЗапроса = "getInboundStatusHistory" Тогда // api v1
		ОтветXdto["inboundStatusHistories"] = СодержимоеОтвета;

	ИначеЕсли ТипЗапроса = "getInboundHistory" Тогда
		ОтветXdto["inboundStatusHistory"] = СодержимоеОтвета;
...
Функция СодержимоеОтветаВСтрокуXML(ОбъектXdto) Экспорт
	...
	// принудительно в <root> возвращаем пространство
	ДанныеXML = дсфОбщегоНазначенияЯндексМаркет.XMLСтрокаСПространствомИменВУзлеRoot(ДанныеXML);
