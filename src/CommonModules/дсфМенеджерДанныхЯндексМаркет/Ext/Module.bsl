...
Процедура СоздатьОбновитьРеестрПриемкиЯндексМаркет(ДанныеРеестра, Результат) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеРеестра["date"]) Тогда
	...
	ОписаниеРеестра.НомерКИС = ИдентификаторЗаявки["yandexId"];
...
Процедура ПодготовитьСведенияОСтатусахЗаявокНаПриемку(ИдентификаторыЗаявокНаПриемку, Результат) Экспорт
	...
	Для каждого ОписаниеЗаявки Из ИдентификаторыЗаявокНаПриемку Цикл
		...
		Если Заявка = Неопределено Тогда
			ТекстОшибки = СтрШаблон("Заявка на приемку с yandexId %1 не была ранее зарегистрирована.",
									ИдентификаторЗаявкиВнешний);
	...
	Если РезультатЗапросаСтатусов.Успех Тогда
		...
		ТипОбъекта = ФабрикаXDTO.Тип("http://www.dsf-yandex-market.kz", "InboundStatuses");
		ДанныеОтвета = ФабрикаXDTO.Создать(ТипОбъекта);
		...
		Для каждого ОписаниеСтатуса Из РезультатЗапросаСтатусов["Статусы"] Цикл
			...
			СведенияОСтатусе["statusCode"] = дсфСтатусыДокументовЯндексМаркет.КодСтатусаПриемкиПоСтатусуДокумента(СтатусДокумента);
...
Процедура ПодготовитьСведенияОСтатусахЗаявокНаПриемкуВариант2(ИдентификаторыЗаявокНаПриемку, Результат) Экспорт
...
Процедура ПодготовитьИсториюСтатусовЗаявокНаПриемку(ИдентификаторыЗаявокНаПриемку, Результат) Экспорт
	...
	Если РезультатЗапросаИсторииСтатусов.Успех Тогда
		...
		ТипОбъектаСтатусПриемки = ФабрикаXDTO.Тип("http://www.dsf-yandex-market.kz", "Status");
		...
		Для каждого ДанныеПриемки Из РезультатЗапросаИсторииСтатусов["ИсторияСтатусовПриемок"] Цикл
			...
			Для каждого ОписаниеСтатуса Из ДанныеПриемки["Статусы"] Цикл
				...
				ИсторияСтатусов["status"].Добавить(ЭлементСпискаСтатусов);
...
Процедура ПодготовитьИсториюСтатусовЗаявкиНаПриемкуВариант2(ИдентификаторЗаявкиНаПриемку, Результат) Экспорт
	...
	Если РезультатЗапросаИсторииСтатусов.Успех Тогда
		...
		ТипОбъекта = ФабрикаXDTO.Тип("http://www.dsf-yandex-market.kz", "InboundStatusHistory");
		ДанныеОтвета = ФабрикаXDTO.Создать(ТипОбъекта);
		...
		ДанныеОтвета["inboundId"] = СведенияОбИдентификаторах;
...
Процедура СоздатьОбновитьЗаявкуНаОтгрузкуЯндексМаркет(ДанныеЗаявки, Результат) Экспорт
	...
	КодТипаОтправки = ДанныеЗаявки["outboundType"];
	ОписаниеЗаявки.ТипОтправки = дсфОбщегоНазначенияЯндексМаркет.ТипОтправкиПоЧисловомуКоду(КодТипаОтправки);
...
Процедура ПодготовитьСведенияОСтатусахЗаявокНаОтгрузкуВариант2(ИдентификаторыЗаявокНаОтгрузку, Результат) Экспорт
...
Процедура ПодготовитьИсториюСтатусовЗаявокНаОтгрузку(ИдентификаторыЗаявокНаОтгрузку, Результат) Экспорт
	...
	Если РезультатЗапросаИсторииСтатусов.Успех Тогда
		...
		Для каждого ДанныеОтгрузки Из РезультатЗапросаИсторииСтатусов["ИсторияСтатусовОтгрузок"] Цикл
			...
			СведенияОбИдентификаторах["yandexId"] = ДанныеОтгрузки["ИдентификаторВнешний"];
			СведенияОбИдентификаторах["partnerId"] = ДанныеОтгрузки["ИдентификаторДокумента"];
			СведенияОбИдентификаторах["fulfillmentId"] = ДанныеОтгрузки["ИдентификаторСклада"];
			
			ЭлементСпискаОтрузок["outboundId"] = СведенияОбИдентификаторах;
								
			Для каждого ОписаниеСтатуса Из ДанныеОтгрузки["Статусы"] Цикл
...
Процедура ПодготовитьИсториюСтатусовЗаявкиНаОтгрузкуВариант2(ИдентификаторыЗаявкиНаОтгрузку, Результат) Экспорт
	...
	Если РезультатЗапросаИсторииСтатусов.Успех Тогда
		...
		ДанныеОтгрузки = РезультатЗапросаИсторииСтатусов["ИсторияСтатусовОтгрузок"][0];
